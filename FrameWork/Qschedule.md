...
这里要注意Quatrz对日和周这两个字段有强制约束：两个字段不能同时为*必须有一个写？

因为这两个字段存在语义冲突，如果同时指定每月15号和每周三，其实无法确定以哪个为准，？的意思是不关心/不指定该字段，来明确消除歧义
...
Qschedule采用自注册方式 代码编写完发布后 应用会自动将任务汇报到调度平台 然后就可以在调度平台上找到对应的任务 进行调度

建议 在类上添加注释，确保被spring扫描，在方法上添加@QSchedule标签并加上job_name(全局唯一)

确保当前类中只有一个方法添加该注解



信安要求从测试环境网络访问办公环境网络被全面禁止，因此只能使用单侧进行本地功能测试，调度系统相关配置要到测试环境发布后测试
...
qconfig认为直接kill掉一个任务是很危险的行为，因此采用了协商停止任务的方法，即在管理后台停止任务时调度中心会发送停止信息给执行任务的机器，机器收到消息后会设置一个标记位，任务代码根据消息位的取值然后决定是否停止自己的工作，未响应标记只能通过重启进程来停止任务



**任务监控**

TaskMonitor通过getLogger获取到Logger对象写日志会收集到调度中心
...
通过给执行任务的方法加一个qunar.tc.schedule.Parameter类型的参数 可以获取任务执行过程中通过任务调度中心传来的参数
