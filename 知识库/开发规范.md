...
好的代码本身就是一种注释，应避免无效的长注释，重视代码规范与质量
...
3、在opt/settings下有server.properties 这个就是本地的文件
...
还有一些集群的信息group.properties

group.name=overseas-backend-fat-captain-100055203
group.id=71188195
...
then
  RESERVED_MEM=374
elif [ $JVM_MAX_MEM -eq 2048 ] || [ $JVM_MAX_MEM -eq 3072 ] || [ $JVM_MAX_MEM -eq 4096 ] || [ $JVM_MAX_MEM -eq 5120 ]
then
  RESERVED_MEM=800
...
if [ $JVM_MAX_MEM -ge 4096 ]; then
  sed -i "s/maxThreads=\"512\"/maxThreads=\"1024\"/g" $CATALINA_BASE/conf/server.xml
fi
...
if [ -r "$CATALINA_BASE/bin/extraenv.sh" ]; then
  . "$CATALINA_BASE/bin/extraenv.sh"
fi
...
JAVA_OPTS="${JAVA_OPTS} \
           ...
           -Dcom.sun.management.jmxremote.password.file=/opt/secret/jmxremote.password \
           -Dcom.sun.management.jmxremote.password.toHashes=false \
           -Dcom.sun.management.jmxremote.ssl=false \
           -DAPPLOGDIR=${SERVER_LOG}/applog \
...
unset LD_PRELOAD
JAVA_OPTS="${JAVA_OPTS} -Djava.util.concurrent.ForkJoinPool.common.threadFactory=com.ctrip.forkjoinworkerthreadfactory.CtripForkJoinWorkerThreadFactory"
```
