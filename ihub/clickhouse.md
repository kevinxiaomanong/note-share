...
不支持事务、频繁删除&修改、不擅长大表join千万级别（因为是share Nothing架构）这样其实就会需要把join的中间结果发给其他节点

share-Nothing架构每个节点都完全独立，拥有自己的计算、存储和内存资源，节点之间没有共享的硬件资源，

**为什么不支持大表join**

1. 内存消耗 join操作通常需要将数据加载到内存进行处理，如果参与join操作的表非常大可能导致超过物理内存的限制，就需要内存和磁盘换入换出，影响性能
2. 网络开销，join操作可能需要在不同节点之间传输大量数据，有网络延迟和开销
3. 数据分布，如果数据在不同节点数据分布不均匀，join操作可能会导致某些节点称为瓶颈
...
例如where id = A003   [A003,A003]

where id > A000 (A000,+inf)

where id like 'A006%'  [A006,A007)

ps：其实这个例子就能很好地解释为什么前缀匹配能够命中索引

2、递归交集判断

3、合并MarkRange区间

每个区间拆分为8个子区间，由merge_tree_coarse_index_granularity指定 默认为8
...
二级索引：以前我们是一个granularity去跳的，现在每三个granularity记录一个minmax，然后去跳，即跳的过程中变快，MergeTree支持多种跳数索引，分别是minmax、set和tokenbf_v1
...
mrk文件：从前面截图可以看到对于CK这样一个列存储db，每一列是存在一起的，每个column对应有.bin文件

然后还有一个mrk文件，这个其实就是标记文件
...
![image-20241226162029049](D:\note\工作文档\ihub\assets\image-20241226162029049.png)

背景：假设有张表以CounterID为排序键，然后想找到CounterID为3267的某一列的数据
